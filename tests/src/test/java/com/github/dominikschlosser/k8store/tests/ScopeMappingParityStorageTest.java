...
public class ScopeMappingParityStorageTest {
    ...
    public void realmRoleScopeMappingOnClientLandsInCustomResource() {
        ...
        assertTrue(realm.admin().clients().get(clientDbId).getScopeMappings().realmLevel().listAll().stream()
        ...
        assertTrue(crMappings.contains(role.getName()),
                "client CR realmScopeMappings must contain the role name");

        realm.admin().clients().get(clientDbId).getScopeMappings().realmLevel().remove(List.of(role));
        assertTrue(realm.admin().clients().get(clientDbId).getScopeMappings().realmLevel().listAll().isEmpty(),
        ...
        assertTrue(afterRemove == null || !afterRemove.contains(role.getName()),
    ...
    public void clientRoleScopeMappingOnClientLandsInCustomResource() {
        ...
        RoleRepresentation clientRoleRep =
                realm.admin().clients().get(roleOwnerDbId).roles().get("scope-mapped-client-role").toRepresentation();

        String consumerDbId = createClient("scope-map-consumer");
        realm.admin().clients().get(consumerDbId).getScopeMappings()
                .clientLevel(roleOwnerDbId).add(List.of(clientRoleRep));

        assertTrue(realm.admin().clients().get(consumerDbId).getScopeMappings()
                        .clientLevel(roleOwnerDbId).listAll().stream()
        ...
        assertTrue(crMappings.getOrDefault("scope-map-role-owner", List.of())
                        .contains("scope-mapped-client-role"),
    ...
    public void realmRoleScopeMappingOnClientScopeLandsInCustomResource() {
        ...
        realm.admin().clientScopes().get(scopeId).getScopeMappings().realmLevel().add(List.of(role));

        assertTrue(realm.admin().clientScopes().get(scopeId).getScopeMappings().realmLevel().listAll().stream()
        ...
        assertTrue(crMappings.contains(role.getName()),
                "client scope CR realmScopeMappings must contain the role name");
    }
}
