...
Requires **Keycloak nightly** with the `cacheless` feature. See [ARCHITECTURE.md](ARCHITECTURE.md)
for the design and the full details behind everything below.

```yaml
apiVersion: keycloak.k8store.io/v1alpha1
...
`keycloak.k8store.io` API group (see `deploy/20-rbac.yaml`).
...
```
--features=cacheless                             # required (nightly)
...
20 kinds under `keycloak.k8store.io/v1alpha1` (manifests in `crds/`, regenerated by
...
`scripts/crd-diff.sh` classifies the changes (compatible vs breaking) and applies them
